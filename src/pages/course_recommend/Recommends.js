...
const Recommends = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3333/admin-recommends/view")
      .then((response) => {
  ...
  const handleCourse = (onsite_id) => {
    window.location = `/online-course/${onsite_id}`;
  };
        ...
        {onsiteData.map((onsite) => (
          <div key={onsite.id} className="rounded overflow-hidden shadow-lg">
            ...
            <div className="px-6 py-4">
              ...
              <p className="text-gray-700 text-base">{onsite.onsite_details}</p>

              <div className="flex justify-center mt-10">
                <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                  <a
                    onClick={() => handleCourse(onsite.onsite_id)}
